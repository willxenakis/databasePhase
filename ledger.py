...
from SimpleCoinComplete import Block
...
ledger.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///test.db'
db = SQLAlchemy(ledger)

class blockChains(db.Model):
    id = db.Column(db.Integer, autoincrement=True)
    systemPub = db.Column(db.String(160), primary_key=True)
    systemPriv = db.Column(db.String(160))
...
class transactions(db.Model):
    ...
    senderPublic = db.Column(db.String(160), nullable=True)
    senderName = db.Column(db.String(80))
    receiverPublic = db.Column(db.String(160), nullable=False)
    receiverName = db.Column(db.String(160))
    operation = db.Column(db.Integer, nullable = False)
    timeStamp = db.Column(db.Integer)
    _hash = db.Column(db.String(160))

    def __repr__(self):
        return '<User %r>' % self.transactionId

class blocks(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    blockChainId = db.Column(db.Integer)
    merkleRoot = db.Column(db.String(160), nullable=False)
    timeStamp = db.Column(db.Integer)
    blockIndex = db.Column(db.Integer)
    nonce = db.Column(db.Integer)
    currHash = db.Column(db.String(160), nullable = False)
    prevHash = db.Column(db.String(160))

    def __repr__(self):
        return '<User %r>' % self.blockIndex

class peers(db.Model):
    id = db.Column(db.Integer)
    address = db.Column(db.String(80), nullable=False)
    name = db.Column(db.String(80))
    publicKey = db.Column(db.String(160), nullable=False, primary_key=True)

    def __repr__(self):
        return '<User %r>' % self.publicKey
...
#Reading blocks and transactions from database and creating blockchain
#Is for each guaranteed to go in order in python?

for block in blocks.query.order_by(blocks.blockIndex):
    blockTransactions = []

    for transaction in transactions.query.filter(transactions.blockId == block.blockIndex).order_by(transactions.order):
        temp = Transaction(Wallet(transaction.receiverName, transaction.receiverPublic, ""), transaction.operation, 
        Wallet(transaction.senderName, transaction.senderPublic, ""), timestamp=transaction.timeStamp, _hash=transaction._hash)
        blockTransactions.append(temp)

    if(block.blockIndex==0):
        #Create head block here
        head = Block(blockTransactions, block.blockIndex, prevBlock=None, nonce=block.nonce, timeStamp=block.timeStamp, 
        merkleRoot=block.merkleRoot, prevHash=block.prevHash, currHash=block.currHash)

        simpleCoin=blockChain(creator, head=head)
    else:
        newTail = Block(blockTransactions, block.blockIndex, prevBlock=simpleCoin.tail, nextBlock=None, nonce=block.nonce, timeStamp=block.timeStamp, 
        merkleRoot=block.merkleRoot, prevHash=block.prevHash, currHash=block.currHash)
        simpleCoin.addBlockManual(newTail)


    # simpleCoin = blockChain(Wallet("", blockChains.query[0].systemPub, ""), 

#If database does not exist, create database with genesis block
if(simpleCoin==None):
    db.session.add(blockChains(id=1, systemPub=creatorKey, systemPriv=creatorPrivKey))
    db.session.commit()
    simpleCoin = blockChain(creator)
    firstBlock = simpleCoin.head
    firstTransaction = firstBlock.data[0]

    db.session.add(blocks(blockIndex=firstBlock.index, merkleRoot=firstBlock.merkleRoot, timeStamp=firstBlock.timeStamp,nonce=firstBlock.nonce,
    blockChainId=1, currHash=firstBlock.currHash, prevHash=firstBlock.prevHash))

    db.session.add(transactions(blockId=0, order=0, senderPublic=firstTransaction.senderPub, receiverPublic=firstTransaction.receiverPub,
    operation=firstTransaction.op, timeStamp=firstTransaction.timeStamp, _hash=firstTransaction.hash))

    db.session.commit()
    
#Validate chain after created
print("HAERHAE;KLFJADSL;F")
print("Chain Verified: " + simpleCoin.verifyChain())

#Reading peers from database and creating peerList
for peer in peers.query:
    temp = Peer(peer.address, Wallet(peer.name, peer.publicKey, ""))
    peerList.append(temp)
...
def test1():
    if request.method == 'POST':
        ...
        if self.isDuplicate(ip):
            ...
            jsonPeerList = json.dumps(tosend, indent=4, cls=ChainEncoder)
            return jsonPeerList
    else:
